...
export default function sortPartCollection_Barrel(direction: "asc" | "desc", sortBy:SortingTypesPart_Barrel){
    const ascending = direction === "asc"
        ...
        if(sortBy === "alphabetical"){
            return ascending ?
                asc((sql`COALESCE(NULLIF(${schema.partCollection_Barrel.manufacturer}, ""), ${schema.partCollection_Barrel.model})`))
                :
                desc((sql`COALESCE(NULLIF(${schema.partCollection_Barrel.manufacturer}, ""), ${schema.partCollection_Barrel.model})`))
        }
        if(sortBy === "createdAt"){
            return ascending ?
