...
export default function BottomBar_ReloadingCollection({handleNavigation, bottomBarRef}:Props){
    ...
    return(
        ...
        >
            <Text variant={"headlineSmall"} style={{width: "100%", textAlign: "center"}}>
                {mainCollectionCategories.reloadingCollection[language]}
            </Text>

            <Divider style={{height: 2, width: "100%", backgroundColor: theme.colors.primary}} />

            {screenNameParamsReloading.map((collection, index) => {
                return(
                    <TouchableOpacity 
